...
static void wss_server_send_messages(httpd_handle_t *server,int send_mode)//send_mode 0:����rgbֵ��1:����CAN����Ϣ��
{
...
static esp_err_t echo_handler(httpd_req_t *req)
{
    ...
    if (strncmp((char *)ws_pkt.payload, "1234", strlen("1234")) != 0)//�ͻ��˸��������˷���1234ʱ��ֻ��������ͻ��˵�ֵ���������Ǹ������еĿͻ���ֵ
    {
    ...
    else if (data_read > 0)
    {
        if (image_header_was_checked == false)
        {
            ...
            if (data_read > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t))
            {
                ...
                image_header_was_checked = true;
                //text_out("                ",0,0);text_out("run...to ",0,0);text_out(new_app_info.version,72,0);//������invalidate();����������������е�scroll_x��ͻ
                err = esp_ota_begin(update_partition, OTA_WITH_SEQUENTIAL_WRITES, &update_handle);
                if (err != ESP_OK)
                {
                    ESP_LOGE(TAG, "esp_ota_begin failed (%s)", esp_err_to_name(err));
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(gpio_input_test, "gpio_input_test", 4096, NULL, 5, NULL,1);
    ...
    for(i=0;i<16;i++)
    {
        printf("BEFORE addr[%8.8x]=%d\n",(unsigned int)p,p[i]);
    }
    p = (volatile int *)(SOC_EXTRAM_DATA_HIGH - exram_size);
    for(i=0;i<1024;i++)
    {
        p[i]=i;
    }
